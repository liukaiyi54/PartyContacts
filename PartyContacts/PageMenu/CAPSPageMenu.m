...
NSString * const CAPSPageMenuOptionICDMenuLeftMargin                    = @"icdMenuLeftMargin";
...
NSString * const CAPSPageMenuOptionSelectedMenuItemLabelBackgroundColor = @"icdSelectedMenuItemLabelBackgroundColor";
...
- (instancetype)initWithViewControllers:(NSArray *)viewControllers frame:(CGRect)frame options:(NSDictionary *)options
{
    ...
    if (options) {
        for (NSString *key in options) {
            ...
            } else if ([key isEqualToString:CAPSPageMenuOptionICDMenuLeftMargin]) {
                _icdMenuLeftMargin = [options[key] floatValue];
            ...
            } else if ([key isEqualToString:CAPSPageMenuOptionSelectedMenuItemLabelBackgroundColor]) {
                _icdSelectedMenuItemLabelBackgroundColor = options[key];
...
- (void)initValues
{
    ...
    _icdSelectedMenuItemLabelBackgroundColor = [UIColor whiteColor];
...
- (void)configureUserInterface
{
    ...
    if (_useMenuLikeSegmentedControl) {
        ...
        _menuScrollView.contentSize = CGSizeMake((_menuItemWidth + _menuMargin) * (CGFloat)_controllerArray.count + _icdMenuLeftMargin, _menuHeight);
    }
    ...
    for (UIViewController *controller in _controllerArray) {
        ...
        if (_useMenuLikeSegmentedControl) {
            menuItemFrame = CGRectMake(self.view.frame.size.width / (CGFloat)_controllerArray.count * (CGFloat)index, 0.0, (CGFloat)self.view.frame.size.width / (CGFloat)_controllerArray.count-20, _menuHeight);
            ...
            menuItemFrame = CGRectMake(_totalMenuItemWidthIfDifferentWidths + _icdMenuLeftMargin + (_menuMargin * index), 0.0, _menuItemWidth, _menuHeight);
            ...
            if (_centerMenuItems && index == 0.0) {
                ...
                menuItemFrame = CGRectMake(_startingMenuMargin + _icdMenuLeftMargin, 0.0, _menuItemWidth, _menuHeight);
            } else {
                menuItemFrame = CGRectMake(_menuItemWidth * index + _menuMargin * index + _icdMenuLeftMargin + _startingMenuMargin, 0.0, _menuItemWidth, _menuHeight);
            }
        }
    ...
    if (_menuItemWidthBasedOnTitleTextWidth) {
        _menuScrollView.contentSize = CGSizeMake((_totalMenuItemWidthIfDifferentWidths + _icdMenuLeftMargin) + (CGFloat)_controllerArray.count * _menuMargin, _menuHeight);
    }
    ...
    if (_mutableMenuItems.count > 0) {
        if ([_mutableMenuItems[_currentPageIndex] titleLabel] != nil) {
            ...
            [_mutableMenuItems[_currentPageIndex] titleLabel].backgroundColor = _icdSelectedMenuItemLabelBackgroundColor;
        }
    }
    ...
    if (_useMenuLikeSegmentedControl) {
        selectionIndicatorFrame = CGRectMake(0.0, _menuHeight - _selectionIndicatorHeight, self.view.frame.size.width / (CGFloat)_controllerArray.count-20.0, _selectionIndicatorHeight);
    } else if (_menuItemWidthBasedOnTitleTextWidth) {
        selectionIndicatorFrame = CGRectMake(_icdMenuLeftMargin, _menuHeight - _selectionIndicatorHeight, [_mutableMenuItemWidths[0] floatValue], _selectionIndicatorHeight);
    } else {
        if (_centerMenuItems) {
            selectionIndicatorFrame = CGRectMake(_startingMenuMargin + _icdMenuLeftMargin, _menuHeight - _selectionIndicatorHeight, _menuItemWidth, _selectionIndicatorHeight);
        } else {
            selectionIndicatorFrame = CGRectMake(_icdMenuLeftMargin, _menuHeight - _selectionIndicatorHeight, _menuItemWidth, _selectionIndicatorHeight);
        }
    }
    ...
    _selectionIndicatorView.layer.cornerRadius = _selectionIndicatorHeight / 2.0;//圆角－by Wenky
    ...
    if (pageIndex >= 0 && pageIndex < _controllerArray.count) {
        [UIView animateWithDuration:0.15 animations:^{
            ...
            if (self.useMenuLikeSegmentedControl) {
                ...
                selectionIndicatorX += _icdMenuLeftMargin;
            ...
            if (self.menuItems.count > 0) {
                if ([self.menuItems[self.lastPageIndex] titleLabel] != nil && [self.menuItems[self.currentPageIndex] titleLabel] != nil) {
                    ...
                    [self.menuItems[self.lastPageIndex] titleLabel].backgroundColor = [UIColor whiteColor];
                    [self.menuItems[self.currentPageIndex] titleLabel].textColor = self.selectedMenuItemLabelColor;
                    [self.menuItems[self.currentPageIndex] titleLabel].backgroundColor = self.icdSelectedMenuItemLabelBackgroundColor;
                }
            }
        }];
    }
    ...
    if (tappedPoint.y < self.menuScrollView.frame.size.height) {
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            if (!(tappedPoint.x >= menuItemLeftBound && tappedPoint.x <= menuItemRightBound)) {
                for (NSInteger i = 1; i<_controllerArray.count; i++) {
...
- (void)viewDidLayoutSubviews
{
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))){
        ...
        if (_useMenuLikeSegmentedControl) {
            ...
            _selectionIndicatorView.frame =  CGRectMake(selectionIndicatorX, self.selectionIndicatorView.frame.origin.y, selectionIndicatorWidth-20, self.selectionIndicatorView.frame.size.height);
            ...
            for (MenuItemView *item in _mutableMenuItems) {
                if (index == 0) {
                    item.frame = CGRectMake(_startingMenuMargin + _icdMenuLeftMargin, 0.0, _menuItemWidth, _menuHeight);
                } else {
